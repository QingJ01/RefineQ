...
This endpoint is a deliberately small product-evaluation surface. It is routed to one existing
RefineQ account but operates only on a dedicated resettable simulation workspace. It does not
read, reset, or update the account's existing learning spaces or their mastery records.
...
REFINEQ_MCP_INTERNAL_SECRET=<at least 32 random characters>
REFINEQ_MCP_ACCOUNT_EMAIL=learner@example.com
REFINEQ_MCP_ALLOWED_HOSTS=learn.example.com
```

The bound account must already exist. The internal secret is never sent by a client; it only
signs opaque run and idempotency tokens. Keep the default five-minute run TTL unless the
evaluator genuinely needs more time. Startup fails closed without the account, a strong
internal secret, or an exact Host allowlist.

Both `https://<host>/mcp` and `https://<host>/api/mcp` are supported. Clients connect without
Authorization headers. The server-bound account email is returned by `refineq_begin_demo` so
operators can verify routing. Never put `REFINEQ_MCP_INTERNAL_SECRET` in a client, URL, shell
history, screenshot, or support log.
...
logs retain the internal signing secret, run tokens, client keys, owner IDs, emails, prompts,
answers, or material text.
...
`rate_limited`, `demo_busy`, `run_not_found`, `run_expired`, `run_completed`,
`idempotency_conflict`, `state_conflict`, `material_required`, and `internal_error`.
Internal exception messages, internal secrets, learner answers, and material contents are not
...
$env:REFINEQ_MCP_ACCOUNT_EMAIL = "learner@example.com"
...
does not print the internal signing secret or run token.
...
Set `REFINEQ_MCP_ENABLED=false` and redeploy. Caddy will still forward the MCP paths, but the API
has no matching route and returns 404. Rotate the internal signing secret when changing the
bound account. Existing hashed run and idempotency rows contain no recoverable raw token and
age out according to their retention policy.
