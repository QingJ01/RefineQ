...
> 日期：2026-08-09
>
> 状态：设计已确认，待实施（v2 已按设计审核补齐边界与成本约束）
>
...
| 版本 | 变更 |
| --- | --- |
| v1 | 初稿：调度协议、六类结果、写操作权限、事件与测试策略 |
| v2 | 按设计审核补齐：空间创建按信号强度分流（保住"一句话就能开始"）、跨空间候选上限与投影查询、分段延迟预算、`auto_navigate` 契约收敛、模型不可用时学习者出口、越界回收路径、输入上限依据、既有 E2E 迁移 |
...
产品目标第一条是"一句话就能开始，用户全程没有建项目这类管理动作"（[03-prd.md](../product/03-prd.md) §3），演示脚本 0:30–0:45 也建立在"说一句话，空间和计划当场出现"上。**对所有输入一律加一步确认会推翻这个承诺。**

因此按长期信号的强度分两条路，而不是一视同仁：

**A. 信号无歧义 → 直接创建，事后可撤销。**

判定条件（确定性规则，不由模型决定）：意图中同时出现**明确的学习对象**与**至少一项时间约束**（具体考试名 / 绝对或相对截止日期 / 每日分钟数），或出现明确的跨天承诺（"两个月系统学习线性代数"）。

此时直接走 `commit_resolution` 并导航，复用既有的路由通知条：展示动作、理由、置信度，并提供**撤销**。撤销机制已实现且有测试覆盖，它本来就是为这个场景设计的。这条路径与当前行为一致，演示动线不变。

**B. 信号模糊 → `propose_workspace` 预览卡。**

只要缺少时间约束、学习对象宽泛（"学一下概率"）、或与既有空间存在多个接近候选，就先出预览卡，显示并允许编辑：
...
**分流规则必须是确定性的**，写在 `policy.py` 里并有表驱动测试。模型只能在规则判定为"模糊"时参与澄清，不能把 A 类降级成 B 类，也不能把 B 类提升成 A 类。这样"防止一次性提问误建空间"的真实目标仍然达成——一次性提问天然不含时间约束，落在 B 类或 `direct_answer`。
...
**越界必须可回收。** 学习与非学习的边界有大量真实的灰色案例（"帮我改一下论文摘要"算不算学习任务？），分类器必然会误判一部分。越界卡因此要提供一个转化入口：

> 这看起来不是学习任务，我暂时不处理。
> [这其实是我的学习任务] [换一个问题]

点击"这其实是我的学习任务"不重新分类，而是直接进入 §5.7 的人工选择：直接解释、选择已有空间、创建新空间。用户的显式声明优先于分类器判断——但仍然受 §4.2 的禁止范围约束（实时事实、医疗法律投资、通用出题判分不因为用户声称"这是学习"就放行）。

§14.2 的"越界率"指标应同时统计**越界后被用户回收的比例**：该比例偏高说明边界划得太窄或入口文案有误导。
...
不传入材料正文、聊天记录、完整计划、题目或答案。归档空间默认不参与，除非用户明确点名。

**候选数量上限为 8**，按 `last_active_at` 倒序截断，不使用空间配额上限（100）。理由见 §7.4：每个候选的 `next_action_*` 字段都有真实读取成本，把 100 个空间摘要送进模型既昂贵又无助于决策——用户在主页要的是"接下来做哪一个"，不是全量清单。

被截断时，结果 `limitations` 中追加一条说明（"仅比较了最近活跃的 8 个学习空间"），并在跨空间调度卡上提供"查看全部空间"链接回主页列表。显式点名的空间即使不在前 8 名也必须纳入候选。
...
**延迟预算（两次调用是串行的，必须分段设限）：**

| 阶段 | 超时 | 重试 | 超时后行为 |
| --- | --- | --- | --- |
| 分类器 | 5 s | 0 | **降级到确定性路由**，不报错：命中已有空间则 `open_workspace`，否则 `clarify` |
| 答案生成器 | 15 s | 1 | 保留输入，显示重试；不返回模板答案 |
| 单次调度总预算 | ≤ 20 s | — | 超出即按上面两条分别降级 |

分类器超时降级而不是失败，是因为它的职责（分类）有确定性替代；答案生成没有替代，只能重试。这与既有教练链路的取舍一致：能确定性完成的绝不因模型不可用而中断。

`direct_answer` 的 P50 / P90 目标：P50 ≤ 6 s、P90 ≤ 18 s，写入 §14.2 指标表。
...
### 7.4 NextAction 复用与聚合成本

跨空间调度复用现有服务端 `NextAction`，不在主页重新实现优先级。

**必须先解决聚合成本。** 现有 `WorkspaceService.next_action(...)` 为**单个**空间计算一次要付：`workspaces.get` + `learning.get`（完整学习记录，含 plan、evidence、question_history、saved_questions）+ `learning_service.progress`（再读一次同一条记录）+ `knowledge.list_materials`——约 4 次读取、2 次完整学习记录反序列化。

主页是登录后的第一个页面，直接对 N 个空间循环调用会让它成为全站最贵的接口。约束：

1. **候选上限 8**（§6.3），不按配额上限展开。
2. **新增投影读取路径** `load_dispatch_summaries(owner_id, workspace_ids)`，一次查询返回每个空间的：计划摘要（今日会话、最近到期复习、考试日）、材料状态计数、最低掌握度主题名。**不走完整 `learning.get`**，也不重复调用 `progress`。
3. `select_next_action` 的输入保持不变，由投影结果装配，确保主页与空间内算出的 NextAction 完全一致。
4. 加一条性能回归测试：8 个候选的调度请求，数据库读取次数有上限断言。
...
**12000 字符的依据与代价。** 上限取自 §4.1 允许的"对本次粘贴文本做总结 / 翻译 / 改写"——一段需要总结的讲义节选通常在数千字，1200 字（教练单条消息量级）不够用。但要明确两点：

1. 这**不是**把主页变成文档处理器。§2.2 的"不接受主页文件上传"边界不变；超过上限时提示"内容较长，建议作为资料上传到学习空间"，把用户导向真正的资料库而不是截断。
2. 长文本只进入**答案生成器**，不进入分类器。分类器只接收前 500 字符做意图判断——粘贴内容不影响"这是不是一次性任务"的分类，却会显著抬高分类成本。这一条要写进 §7.2 的两步隔离实现。
...
`match_kind` 取 `explicit_command | semantic_match`，由服务端的确定性规则判定。**`auto_navigate` 是服务端的最终结论，前端只信这一个字段**——只有 `match_kind == "explicit_command"` 时服务端才会置 `true`。

不要求前端二次判断"是否显式命令"：那是语义判断，前端做不可靠。`match_kind` 只用于卡片文案（说明为什么带你来这里），不参与前端的跳转决策。
...
| 创建空间（信号无歧义） | 中 | 直接创建 + 路由通知 + 撤销（§5.6 A） |
| 创建空间（信号模糊） | 中 | 预览并确认（§5.6 B） |
| 改期 / 调整时长 | 中 | 显示前后差异并确认 |
...
| 场景 | 行为 |
| --- | --- |
| 模型未配置（管理员） | 显式空间导航和确定性跨空间调度可用；直接回答提示前往 `/admin/integrations/chat` |
| 模型未配置（普通学习者） | 同上可用能力；直接回答显示"这类问题需要管理员启用模型"，并给出**进入学习空间**的主按钮。**不得只给一句陈述句**——配置模型是管理员权限，学习者看到"去配置"是死路（既有 `session-coach` 已有同类问题，不要复制） |
...
| 越界率与越界回收率 | 判断学习范围是否过窄或入口文案是否误导；回收率高说明边界划窄了 |
| 直接回答失败率与 P50 / P90 | 评估基础可用性。目标：P50 ≤ 6 s、P90 ≤ 18 s（§7.2） |
| 候选截断发生率 | 空间数超过 8 的用户占比，判断上限是否需要调整 |
| 用户手动改选空间率 | 评估候选排序错误 |
...
- 新空间按 §5.6 分流：信号无歧义直接创建并保留撤销（与当前行为一致），信号模糊只预览；
- 不支持跨空间写操作；
- 主页改为单结果卡；
- 上线无正文事件与指标。
...
`apps/web/tests/e2e/learning-journey.spec.ts` 的第一步是 `learning-intent` → `start-learning` → 断言直接落到 `/learn/:id/today`。主页改为单结果卡后这条**必然失败**。迁移方案：

1. 该 E2E 的意图文本改为**信号无歧义**的句子（含考试与日期），使其走 §5.6 A 路径，断言保持"直接落到 today"不变——这同时成为 A 路径的回归保护。
2. 新增一条 E2E 覆盖 B 路径：模糊输入 → 出现 `propose_workspace` 预览卡 → 确认后才创建。
3. 新增一条 E2E 覆盖 `direct_answer`：提问后停留在主页、不产生空间、刷新后结果消失。
4. `contracts.test.ts` 中断言主页调用 `resolveIntent` 的用例改为断言调用 `dispatch`。

三条 E2E 与 §18 的产品验收标准一一对应，不允许只改断言让旧用例"通过"。
...
- 长期信号无歧义时一句话直接建好空间与计划，并可撤销（保住 PRD 目标一与演示动线）；信号模糊时只返回预览，未经确认不写入。
- 分流由确定性规则决定，模型不能把任一类改判为另一类。
