...
- 标题、目标、截止日和每日时间可以编辑。编辑不会复用旧签名；服务端先校验原预览，再为修改后的字段重新签名。
...
| 多标签重复确认 | 计划被写两次 | token + 版本 + owner-scoped 幂等收据 | 并发不同提案会一成一冲突，符合预期 |
...
自动化已覆盖硬规则、双语合成集、prompt injection、500/12000 字符隔离、答案器无 workspace 上下文、direct answer 不改变学习域、强/弱空间分流、编辑后重新签名、token 篡改/过期/owner 绑定、计划确认前后版本、8 空间读取上限、指标隔离、六类卡片、旧请求防护、移动端/焦点，以及强信号、弱信号和刷新清除答案三条 E2E。
