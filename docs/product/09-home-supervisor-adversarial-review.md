...
- 强信号创建后返回短时、owner-bound 的撤销凭证。撤销会真正删除刚创建且仍未变化的空间；改名、上传资料、开始教练会话或产生学习状态后，状态哈希变化，服务端拒绝撤销并保留空间。状态核对、删除和收据提交持有与教练会话相同的空间事务锁；最终状态核对还在资料恢复租约内完成，避免“核对后、删除前”从另一设备插入新会话或新资料。
