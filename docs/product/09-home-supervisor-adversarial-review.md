...
- 显式打开先解析真实已有空间，再应用“非学习任务”边界；因此名为 `Marketing Copy` 的合法空间仍可被打开，但独立的营销文案请求不会误建空间。
- 其他匹配只展示卡片，由用户点击进入。
...
- 强信号创建后返回短时、owner-bound 的撤销凭证。撤销会真正删除刚创建且仍未变化的空间；改名、上传资料、开始教练会话或产生学习状态后，状态哈希变化，服务端拒绝撤销并保留空间。状态核对、删除和收据提交持有与教练会话相同的空间事务锁；资料恢复租约从最终状态核对一直持有到空间删除与幂等收据的外层事务提交之后，失败则回滚 pending 再释放，避免另一设备在未提交窗口读到旧空间并写入孤儿资料。
...
| “撤销”只回到上一页，或与新会话/资料并发 | 用户误以为空间已删除，或刚写入的数据被一并删掉 | 签名撤销端点 + 创建态哈希 + 空间事务锁 + 资料恢复租约 + 幂等收据 | 空间变化后必须转为显式删除流程 |
| 登出后迟到快照复活旧空间 | 跨账号显示旧状态 | 每次异步提交前复验 auth token；登出同步清空 auth ref | 已完成请求仍消耗少量网络资源，但不能提交 UI 状态 |
| 旧确认覆盖新主页请求 | 用户已问 B，却被 A 的迟到确认强制带走 | 确认、列表读取、快照应用和导航均绑定最新 request generation | 服务端已开始的 A 写入仍可完成，但不会夺取当前 UI |
| 修订冲突后退回旧目标 | 用户修改被静默丢失 | 新预览生成后重新应用结构化 title/goal/deadline/minutes，并再次展示 | 连续外部变化仍会要求继续复核，不自动执行 |
...
- 空日期、纯空白标题/目标和越界每日时间会在前端禁用确认，并由服务端 422 兜底；不会产生浏览器异常或 500。
...
自动化已覆盖硬规则、双语合成集、prompt injection、500/12000 字符隔离、答案器无 workspace 上下文、direct answer 不改变学习域、强/弱空间分流、目标编辑后的语义重算与二次确认、token 篡改/过期/owner 绑定、并发幂等确认、真实撤销及状态变化拒绝、撤销与教练会话/跨设备资料上传的并发边界、计划确认前后版本、8 空间读取上限、指标隔离、六类卡片、旧请求与旧登录态防护、移动端/焦点，以及强信号、弱信号、一次性问答刷新清除、转换冲突恢复、真实撤销和迟到快照五类 E2E。
